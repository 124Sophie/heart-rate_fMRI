...
## t test für unterschiede der herzraten daten 
hist(ecg_data$diff)
hist(ecg_data$diff_inter)
...
ggplot(HR_plot_table, aes(x=block_name,y=bpm, colour=Subject))+
  geom_boxplot()+
  facet_grid(~Drug)


ggplot(ecg_data, aes(x=rel_time,y=bpm, colour=Drug))+
  geom_point(size=0.01)+
  facet_wrap(~Subject)

ggplot(ecg_data, aes(x=block,y=bpm, colour=Drug))+
  geom_boxplot()+
  facet_wrap(~Subject)
...
hist(BOLD_HR$mPFC_K)
hist(BOLD_HR$mPFC_P)
hist(BOLD_HR$bilPC_K)
hist(BOLD_HR$bilPC_P)
...
##hier noch statistik zum BOLD Signal einfügen
#wie unterscheiden sich placebo und ketamin? 
